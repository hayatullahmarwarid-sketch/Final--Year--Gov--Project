...
Path in repo: `back-end/src/server.js`. Together, these exports and any side effects at import time define how the rest of the project interacts with `server.js`.

## Libraries used

- **dotenv** – third-party dependency for this module.
- **node:http** – third-party dependency for this module.
- **./config/env.js** (`{ getEnv }`) – relative project import.
- **./config/logger.js** (`{ getLogger }`) – relative project import.
- **./app.js** (`{ createApp }`) – relative project import.
- **../database/connection/mongoose.js** (`{ connectMongo, disconnectMongo }`) – relative project import.
- **../database/indexes/registry.js** (`{ ensureModelIndexes }`) – relative project import.
- **./config/redis.js** (`{ disconnectRedis, getRedis }`) – relative project import.
- **./jobs/register-in-process-handlers.js** (`{ registerInProcessHandlersIfNeeded }`) – relative project import.
- **./jobs/queue-registry.js** (`{ shutdownQueues }`) – relative project import.

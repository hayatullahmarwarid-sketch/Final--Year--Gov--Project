...
Path in repo: `back-end/src/modules/search/search.routes.js`. Together, these exports and any side effects at import time define how the rest of the project interacts with `search.routes.js`.
...
- **./search.controller.js** (`{ searchController }`) – relative project import.
- **./search.validation.js** (`{ unifiedSearchQuerySchema }`) – relative project import.

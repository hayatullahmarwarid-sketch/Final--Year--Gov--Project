...
# Notifications Routes (`notifications.routes.js`)
...
Path in repo: `back-end/src/modules/notifications/notifications.routes.js`. Together, these exports and any side effects at import time define how the rest of the project interacts with `notifications.routes.js`.

## Libraries used

- **express** – third-party dependency for this module.
- **../shared/http/index.js** (`{ validateRequest }`) – relative project import.
- **./notifications.controller.js** (`{ notificationsController }`) – relative project import.
- **./middleware/notification-recipient.middleware.js** (`{ notificationRecipientMiddleware }`) – relative project import.

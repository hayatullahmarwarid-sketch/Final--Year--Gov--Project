...
# Notification Navigation (`notification-navigation.ts`)
...
Path in repo: `lib/notification-navigation.ts`. Together, these exports and any side effects at import time define how the rest of the project interacts with `notification-navigation.ts`.

## Libraries used

- **expo-router** – third-party dependency for this module.
- **@/contexts/public-user-data-context** (`type { ExamAttemptRecord }`) – shared React context.

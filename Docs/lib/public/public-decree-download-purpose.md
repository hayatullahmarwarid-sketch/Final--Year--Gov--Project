...
# Public Decree Download (`public-decree-download.ts`)
...
Path in repo: `lib/public/public-decree-download.ts`. Together, these exports and any side effects at import time define how the rest of the project interacts with `public-decree-download.ts`.
...
- **@/constants/api** (`{ resolveApiAssetUrl }`) – shared constants.
- **@/lib/adapters/storage** (`{ storage }`) – shared library code.
- **@/lib/api/public-user** (`{ buildPublicDecreePdfUrl }`) – app API and data access helper.

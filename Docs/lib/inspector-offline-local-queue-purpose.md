...
# Inspector Offline Local Queue (`inspector-offline-local-queue.ts`)
...
Path in repo: `lib/inspector-offline-local-queue.ts`. Together, these exports and any side effects at import time define how the rest of the project interacts with `inspector-offline-local-queue.ts`.

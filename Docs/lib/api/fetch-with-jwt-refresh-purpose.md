...
# Fetch With Jwt Refresh (`fetch-with-jwt-refresh.ts`)
...
Path in repo: `lib/api/fetch-with-jwt-refresh.ts`. Together, these exports and any side effects at import time define how the rest of the project interacts with `fetch-with-jwt-refresh.ts`.

## Libraries used

- **@/lib/api/auth-token-refresh** (`{ postBackendTokenRefresh }`) – app API and data access helper.
- **@/lib/api/jwt-session-storage** (`{ getJwtAccessToken, getJwtRefreshToken, saveJwtTokens }`) – app API and data access helper.

...
# Auth Token Refresh (`auth-token-refresh.ts`)
...
Path in repo: `lib/api/auth-token-refresh.ts`. Together, these exports and any side effects at import time define how the rest of the project interacts with `auth-token-refresh.ts`.

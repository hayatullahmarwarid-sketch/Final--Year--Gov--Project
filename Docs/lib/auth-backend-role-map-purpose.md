...
# Auth Backend Role Map (`auth-backend-role-map.ts`)
...
Path in repo: `lib/auth-backend-role-map.ts`. Together, these exports and any side effects at import time define how the rest of the project interacts with `auth-backend-role-map.ts`.

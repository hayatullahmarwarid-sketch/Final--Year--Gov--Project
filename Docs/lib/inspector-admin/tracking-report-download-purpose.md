...
# Tracking Report Download (`tracking-report-download.ts`)
...
Path in repo: `lib/inspector-admin/tracking-report-download.ts`. Together, these exports and any side effects at import time define how the rest of the project interacts with `tracking-report-download.ts`.
...
- **@/lib/dept-upload/reports-file-save** (`{ savePdfFileToDeviceStorage }`) – shared library code.

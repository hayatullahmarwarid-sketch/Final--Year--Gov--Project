...
# Async Storage Adapter (`async-storage-adapter.ts`)
...
Path in repo: `lib/storage/async-storage-adapter.ts`. Together, these exports and any side effects at import time define how the rest of the project interacts with `async-storage-adapter.ts`.

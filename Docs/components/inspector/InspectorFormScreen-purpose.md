...
# InspectorFormScreen (`InspectorFormScreen.tsx`)
...
Path in repo: `components/inspector/InspectorFormScreen.tsx`. Together, these exports and any side effects at import time define how the rest of the project interacts with `InspectorFormScreen.tsx`.

## Libraries used

- **@expo/vector-icons** – third-party dependency for this module.
- **expo-image** – third-party dependency for this module.
- **expo-image-picker** – third-party dependency for this module.
- **expo-location** – third-party dependency for this module.
...
- **@/data/inspector-sync-queue** (`{ type InspectorSyncQueueItem }`) – project module.
- **@/hooks/use-app-translation** (`{ useAppTranslation }`) – custom React hook.
- **@/lib/api/files** (`{ registerInspectionEvidenceFromPicker }`) – app API and data access helper.
...
- **@/lib/inspector-offline-local-queue** (`{ appendLocalOfflineSubmit }`) – shared library code.
- **./SignaturePadModal** (`{ SignaturePadModal }`) – relative project import.

...
# InspectorTaskListScreen (`InspectorTaskListScreen.tsx`)
...
Path in repo: `components/inspector/InspectorTaskListScreen.tsx`. Together, these exports and any side effects at import time define how the rest of the project interacts with `InspectorTaskListScreen.tsx`.
...
- **@/data/inspector-tasks** (`{ type InspectorTask, type TaskStatus }`) – project module.
- **@/contexts/inspector-workspace-context** (`{ useInspectorWorkspace }`) – shared React context.
- **./inspector-chrome** (`{ INSPECTOR_BAR_BG, inspectorHeaderShadow }`) – relative project import.
- **./InspectorBottomNav** (`{ InspectorBottomNav, inspectorBottomNavOffset }`) – relative project import.
- **./inspector-translations** (`{ INSPECTOR_TRANSLATIONS, inspectorDashboardStatusText }`) – relative project import.

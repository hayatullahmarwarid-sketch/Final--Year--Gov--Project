...
# PendingDecreeModals (`PendingDecreeModals.tsx`)
...
Path in repo: `components/dept-upload/PendingDecreeModals.tsx`. Together, these exports and any side effects at import time define how the rest of the project interacts with `PendingDecreeModals.tsx`.
...
- **@/lib/api/decree-upload** (`type { DecreeCategory, LocalizedContentBlock }`) – app API and data access helper.
- **@/lib/api/decree-upload** (`{ getDecreeById }`) – app API and data access helper.

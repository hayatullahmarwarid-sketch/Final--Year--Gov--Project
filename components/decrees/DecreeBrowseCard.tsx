...
type DecreeBrowseCardProps = {
  item: DecreeListItem;
  onView?: () => void;
  onDownload?: () => void | Promise<void>;
};

/** Decrees tab row — parity with zip `Decrees.tsx` `DecreeRow`. */
export function DecreeBrowseCard({ item, onView, onDownload }: DecreeBrowseCardProps) {
  ...
  const cs = DECREE_TAB_CATEGORY_VISUAL[item.categoryId] ?? DEFAULT_TAB_CATEGORY_VISUAL;
  ...
  return (
    <Pressable
      ...
      style={() => [styles.card, tk.card, pressed && styles.cardActive]}>
      <View style={styles.topGradientRow}>
        <View style={[styles.topGradSeg, { backgroundColor: Brand.green }]} />
        <View style={[styles.topGradSeg, { backgroundColor: cs.accent }]} />
